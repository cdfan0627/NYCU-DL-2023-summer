...
class Dataset_Dance(torchData):
    ...
    def __init__(self, root, transform, mode='train', video_len=7, partial=1.0):
        ...
        if mode == 'train':
            png_files = glob(os.path.join(root, 'train/train_img/*.png'))
            self.img_folder     = sorted((os.path.normpath(file_path).replace("\\", "/") for file_path in png_files), key=get_key)
            self.prefix = 'train'
        elif mode == 'val':
            png_files = glob(os.path.join(root, 'val/val_img/*.png'))
            self.img_folder     = sorted((os.path.normpath(file_path).replace("\\", "/") for file_path in png_files), key=get_key)
            self.prefix = 'val'
        else:
