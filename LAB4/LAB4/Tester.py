...
class Dataset_Dance(torchData):
    def __init__(self, root, transform, mode='test', video_len=7, partial=1.0):
        ...
        for i in range(data_num):
            img_files = glob(os.path.join(root, f'test/test_img/{i}/*'))
            self.img_folder.append(sorted((os.path.normpath(file_path_1).replace("\\", "/") for file_path_1 in img_files), key=get_key))
            label_files = glob(os.path.join(root, f'test/test_label/{i}/*'))
            self.label_folder.append(sorted((os.path.normpath(file_path_2).replace("\\", "/") for file_path_2 in label_files), key=get_key))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--save_root',     type=str, default= "C:/Users/user/Desktop/lab4_test",  help="The path to save your data")
    ...
    parser.add_argument('--ckpt_path',     type=str,    default="C:/Users/user/Desktop/best_model.ckpt",help="The path of your checkpoints")   
