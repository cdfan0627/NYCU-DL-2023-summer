...
from dataloader_resnet18_resnet50 import LeukemiaLoader
import time
...
def main():
    ...
    models = [ResNet18()]
    model_names = ['resnet_18']
    train_loader = DataLoader(LeukemiaLoader('train',None), batch_size=64, shuffle=True, num_workers=2)
    valid_loader = DataLoader(LeukemiaLoader('valid',None), batch_size=64, shuffle=False, num_workers=2)
    ...
    for model, model_name in zip(models, model_names):
        ...
        best_valid_acc = 0
        optimizer = optim.SGD(model.parameters(), lr=0.005,weight_decay=0.0001, momentum=0.9)
